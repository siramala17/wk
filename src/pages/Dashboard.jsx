...
const QUICK = [
  { to: '/assessment',      emoji: '📋', label: 'ประเมิน\nสุขภาพ',  grad: ['#1e40af','#3b82f6'], shadow: 'rgba(37,99,235,0.50)' },
  { to: '/bmi',             emoji: '⚖️', label: 'คำนวณ\nBMI',      grad: ['#b45309','#fbbf24'], shadow: 'rgba(245,158,11,0.50)' },
  { to: '/nubcal',          emoji: '🔥', label: 'บันทึก\nแคลอรี่',  grad: ['#c2410c','#fb923c'], shadow: 'rgba(249,115,22,0.50)' },
  { to: '/analytics',       emoji: '📊', label: 'กราฟ\nสุขภาพ',    grad: ['#0e7490','#22d3ee'], shadow: 'rgba(6,182,212,0.50)'  },
  { to: '/recommendations', emoji: '🤖', label: 'แนะนำ\nAI',       grad: ['#065f46','#34d399'], shadow: 'rgba(16,185,129,0.50)' },
  { to: '/knowledge',       emoji: '📚', label: 'ใบ\nความรู้',     grad: ['#4338ca','#a78bfa'], shadow: 'rgba(139,92,246,0.50)'  },
]
    ...
    <>
      ...
      <div className="max-w-2xl mx-auto px-4 pt-4">
        <div className="flex gap-2 p-1 rounded-2xl"
          ...
          {[
            { key: 'home',    icon: <Home size={15} />,        label: 'หน้าหลัก' },
            { key: 'profile', icon: <UserCircle size={15} />,  label: 'บัญชีของฉัน' },
          ].map(({ key, icon, label }) => (
            <button
              key={key}
      ...
      {mainTab === 'home' && (
        <div className="max-w-2xl mx-auto px-4 pt-4 pb-6 space-y-4 animate-fade-in">
          ...
          >
            ...
            <div className="absolute bottom-10 right-6 w-14 h-14 rounded-full animate-float-slow pointer-events-none"
              style={{ background: 'radial-gradient(circle, rgba(165,243,252,0.30), transparent 70%)' }} />

            {hasData ? (
              <div className="relative flex items-center justify-between gap-4">
                <div className="flex-1">
                  <p className="text-blue-200 text-sm mb-0.5 font-medium">สวัสดี, {user.firstName || user.name} 👋</p>
          ...
          {/* ── Quick Actions ── */}
          <div>
            <p className="text-xs font-black text-slate-500 uppercase tracking-wider mb-3 px-0.5">เมนูด่วน</p>
            <div className="grid grid-cols-3 gap-3">
              {QUICK.map(({ to, emoji, label, grad, shadow }) => (
                <Link
                  key={to}
                  to={to}
          ...
          {latestAssessment && (
            <div>
              <div className="flex items-center justify-between mb-2.5 px-0.5">
                <p className="text-xs font-black text-slate-500 uppercase tracking-wider">ข้อมูลล่าสุด</p>
                {bmiData && (
                  <span
          ...
          {history.length > 0 && (
            ...
            >
              ...
              <div className="flex items-center justify-between mb-4 mt-1">
                <div>
                  <p className="font-black text-slate-800 text-sm">แนวโน้มสุขภาพ 7 วัน</p>
          ...
          >
            ...
            <div className="relative z-10">
              <p className="text-sm font-black text-yellow-300">เคล็ดลับสุขภาพวันนี้</p>
