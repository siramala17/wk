...
import { Moon, Smartphone, Brain, Dumbbell, Droplets, Scale, Zap, TrendingUp, BookOpen, Camera, ChevronRight } from 'lucide-react'
...
const STAT_CONFIG = [
  { key: 'sleep',    icon: Moon,       label: 'นอนหลับ',    unit: 'ชม.',       max: 9,  color: '#818cf8', bg: '#eef2ff', getVal: a => a.sleepHours },
  { key: 'screen',   icon: Smartphone, label: 'หน้าจอ',     unit: 'ชม.',       max: 12, color: '#a78bfa', bg: '#f5f3ff', getVal: a => a.screenHours },
  { key: 'stress',   icon: Brain,      label: 'ความเครียด', unit: '/10',        max: 10, color: '#fb923c', bg: '#fff7ed', getVal: a => a.stressLevel },
  { key: 'exercise', icon: Dumbbell,   label: 'ออกกำลัง',  unit: 'วัน/สัปดาห์', max: 7, color: '#34d399', bg: '#ecfdf5', getVal: a => a.exerciseDays },
  { key: 'water',    icon: Droplets,   label: 'ดื่มน้ำ',    unit: 'แก้ว',       max: 8,  color: '#22d3ee', bg: '#ecfeff', getVal: a => a.waterGlasses },
]
...
const QUICK = [
  { to: '/assessment', emoji: '📋', label: 'ประเมิน\nสุขภาพ', from: '#7c3aed', to2: '#4f46e5' },
  { to: '/bmi',        emoji: '⚖️', label: 'คำนวณ\nBMI',     from: '#ec4899', to2: '#f43f5e' },
  { to: '/nubcal',     emoji: '🔥', label: 'บันทึก\nแคลอรี่', from: '#f97316', to2: '#f59e0b' },
  { to: '/analytics',  emoji: '📊', label: 'กราฟ\nสุขภาพ',   from: '#06b6d4', to2: '#3b82f6' },
  { to: '/recommendations', emoji: '🤖', label: 'แนะนำ\nAI', from: '#10b981', to2: '#059669' },
  { to: '/knowledge',  emoji: '📚', label: 'ใบ\nความรู้',    from: '#a78bfa', to2: '#7c3aed' },
]

const CustomTooltip = ({ active, payload, label }) => {
  if (active && payload?.length) {
    return (
      <div className="bg-white border border-violet-100 rounded-xl px-3 py-2 shadow-lg text-xs">
        <p className="text-slate-400">{label}</p>
        <p className="font-bold text-violet-600">{payload[0].value} คะแนน</p>
      </div>
    )
  }
  return null
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 pt-4 pb-6 space-y-4 animate-fade-in">
      ...
      <div className="relative rounded-3xl p-5 text-white overflow-hidden"
        style={{ background: 'linear-gradient(135deg, #6d28d9 0%, #5b21b6 40%, #4338ca 100%)', boxShadow: '0 8px 32px rgba(109,40,217,0.35)' }}>
        ...
        {hasData ? (
          <div className="relative flex items-center justify-between gap-4">
            <div className="flex-1">
              <p className="text-violet-200 text-sm mb-0.5">สวัสดี, {user.firstName || user.name} 👋</p>
              ...
              <div className="flex items-center gap-3 mt-1">
                <div className="flex items-center gap-1 text-xs font-semibold text-violet-200">
            ...
            <div className="relative flex-shrink-0">
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center">
                ...
                <p className="text-[10px] text-violet-200 font-semibold">/ 100</p>
              </div>
            </div>
          </div>
        ) : (
          <div className="relative text-center py-2">
            <p className="text-violet-200 text-sm mb-1">สวัสดี, {user.firstName || user.name} 👋</p>
            <h1 className="text-2xl font-black mb-1">ยังไม่มีข้อมูล</h1>
            <p className="text-violet-200 text-sm mb-4">เริ่มประเมินสุขภาพเพื่อดูคะแนนของคุณ</p>
          </div>
        )}
        ...
        <Link to="/assessment"
          ...
          style={{ background: 'linear-gradient(135deg, #fbbf24, #f59e0b)', color: '#78350f', boxShadow: '0 4px 16px rgba(251,191,36,0.4)' }}>
          <Zap size={16} />
          ...
          {QUICK.map(({ to, emoji, label, from, to2 }) => (
            <Link key={to} to={to}
              ...
              <div className="w-11 h-11 rounded-2xl flex items-center justify-center text-xl"
                style={{ background: `linear-gradient(135deg, ${from}22, ${to2}22)` }}>
                {emoji}
              </div>
      ...
      {latestAssessment && (
        <div>
          <div className="flex items-center justify-between mb-2.5 px-0.5">
            ...
            {bmiData && (
              <span className="text-[11px] font-bold text-pink-500 bg-pink-50 px-2.5 py-1 rounded-full border border-pink-100">
      ...
      {/* ── Weekly Chart ── */}
      {history.length > 0 && (
        <div className="bg-white rounded-2xl p-5 shadow-sm" style={{ border: '1.5px solid rgba(139,92,246,0.1)' }}>
          <div className="flex items-center justify-between mb-4">
            ...
            <Link to="/analytics"
              className="flex items-center gap-1 text-xs font-semibold text-violet-600 bg-violet-50 px-2.5 py-1.5 rounded-xl hover:bg-violet-100 transition-colors">
